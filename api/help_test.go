	...
	tests := []struct {
	...
	}{
		{
			...
			want: strings.Join([]string{
				...
				"`gnu`, `head-in`, `hellokitty`, `kiss`, `kitty`, `koala`, `kosh`, `luke-koala`, `mech-and-cow`, `meow`,",
				"`milk`, `moofasa`, `moose`, `mutilated`, `pony-smaller`, `pony`, `random`, `ren`, `sheep`, `skeleton`,",
				"`snowman`, `sodomized-sheep`, `stegosaurus`, `stimpy`, `suse`, `three-eyes`, `turkey`, `turtle`, `tux`, `unipony-smaller`,",
				"`unipony`, `vader-koala`, `vader`, `www`",
				"",
				"Moods: `borg`, `dead`, `greedy`, `paranoid`, `random`, `stoned`, `tired`, `wired`, `young`",
				"",
			}, "\n"),
		},
	}
...
func Test_getCows(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: []string{
				"`apt`", "`beavis.zen`", "`bong`", "`bud-frogs`", "`bunny`", "`calvin`", "`cheese`", "`cock`", "`cower`", "`daemon`", "`default`", "`dragon-and-cow`", "`dragon`", "`duck`", "`elephant-in-snake`", "`elephant`", "`eyes`", "`flaming-sheep`", "`ghostbusters`", "`gnu`", "`head-in`", "`hellokitty`", "`kiss`", "`kitty`", "`koala`", "`kosh`", "`luke-koala`", "`mech-and-cow`", "`meow`", "`milk`", "`moofasa`", "`moose`", "`mutilated`", "`pony-smaller`", "`pony`", "`random`", "`ren`", "`sheep`", "`skeleton`", "`snowman`", "`sodomized-sheep`", "`stegosaurus`", "`stimpy`", "`suse`", "`three-eyes`", "`turkey`", "`turtle`", "`tux`", "`unipony-smaller`", "`unipony`", "`vader-koala`", "`vader`", "`www`",
			},
		},
	}
	for _, tt := range tests {
...
func Test_getMoods(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: []string{
				"`borg`", "`dead`", "`greedy`", "`paranoid`", "`random`", "`stoned`", "`tired`", "`wired`", "`young`",
			},
		},
	}
	for _, tt := range tests {
